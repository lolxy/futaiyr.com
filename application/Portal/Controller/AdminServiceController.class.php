...
class AdminServiceController extends AdminbaseController {
	...
	// 文章添加
	public function add(){
		$terms = $this->terms_model->where(array("taxonomy"=>"service"))->order(array("listorder"=>"asc"))->select();
	...
	public function edit(){
		...
		$terms=$this->terms_model->where(array("taxonomy"=>"service"))->select();
	...
	private function _lists($where=array()){
		...
		$where['post_type']=array(array('eq',5));  // 1 产品 2 单页 3 案例 4 新闻 5 服务 6 下载
			...
			$term=$this->terms_model->where(array('taxonomy'=>'service','term_id'=>$term_id))->find();
			$this->assign("term",$term);
		}
	...
	private function _getTree(){
		...
		$result = $this->terms_model->where(array('taxonomy'=>'service'))->order(array("listorder"=>"asc"))->select();
